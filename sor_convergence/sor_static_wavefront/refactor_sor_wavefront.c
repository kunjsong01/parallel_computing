...
void update_points(double **x_matrix, double **xnew_matrix, double omega){
	...
	int i, j;
	int slength; // variable to ignore the first  and last two diagonal strips
	int z; // variable to skip the boundary elements in each diagonal strip
	...
	for (i=0; i < (2*N -1); i++){
		...
		for (j=k; j <= i-k; j++){
			...
			if (slength > 2) {
				if ((z != 1) && (z != slength)){
					xnew_matrix[j][i-j] = x_matrix[j][i-j]+0.25*omega*(xnew_matrix[j-1][i-j] + xnew_matrix[j][i-j-1] + x_matrix[j+1][i-j] + x_matrix[j][i-j+1] - (4*x_matrix[j][i-j]));
				}
				z++;
			}
		}
